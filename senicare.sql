...
CREATE TABLE IF NOT EXISTS `senicare`.`tel_auth_number` (
  ...
  `auth_number` VARCHAR(45) NOT NULL COMMENT '인증 번호',
...
CREATE TABLE IF NOT EXISTS `senicare`.`nurses` (
  ...
  `nurses_user_id` VARCHAR(20) NOT NULL,
  PRIMARY KEY (`user_id`),
  UNIQUE INDEX `user_id_UNIQUE` (`user_id` ASC) VISIBLE,
  UNIQUE INDEX `nursescol1_UNIQUE` (`tel_number` ASC) VISIBLE,
  CONSTRAINT `tel_auth`
...
CREATE TABLE IF NOT EXISTS `senicare`.`customers` (
  `custom_number` INT NOT NULL COMMENT '고객 번호',
...
CREATE TABLE IF NOT EXISTS `senicare`.`care_records` (
  `record_number` INT NOT NULL COMMENT '관리 기록 번호',
